...
async function checkUserStatus() {
  try {
    ...
    const [allUsers] = await query('SELECT id, employee_id, username, is_active, role FROM users WHERE employee_id = ?', ['ADMIN001']);
    ...
    // 4. Check is_active value specifically
    const [activeCheck] = await query('SELECT id, employee_id, is_active, role FROM users WHERE id = ?', [1]);
