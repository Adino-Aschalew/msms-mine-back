...
const CreateLoanCommitteeAdmin = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.committee_level) {
      errors.push('Committee level is required')
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (adminData.max_loan_amount) {
        adminData.max_loan_amount = parseFloat(adminData.max_loan_amount)
      }
      await adminService.createLoanCommitteeAdmin(adminData)
      navigate('/admin/loan-committee-admins', { 
        state: { message: 'Loan Committee Admin created successfully' } 
      })
    } catch (err) {
      setError(err.message || 'Failed to create loan committee admin')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="bg-white shadow">
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex items-center py-6">
            <button
              onClick={() => navigate('/admin/loan-committee-admins')}
              className="mr-4 text-gray-500 hover:text-gray-700"
            >
              <ArrowLeft className="h-6 w-6" />
            </button>
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Create Loan Committee Admin</h1>
              <p className="text-sm text-gray-600 mt-1">Add a new loan committee administrator</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      placeholder="committee.admin@company.com"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                      required
                    />
                  </div>
              ...
              <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                ...
                >
                  ...
                  {loading ? 'Creating...' : 'Create Committee Admin'}
                </button>
              </div>
            </form>
          </div>
...
export default CreateLoanCommitteeAdmin
