...
const HRAdminManagement = () => {
  ...
  const fetchHRAdmins = async () => {
    try {
      setLoading(true)
      const response = await adminService.getHRAdmins()
      setHRAdmins(response.data)
    } catch (err) {
      setError(err.message || 'Failed to fetch HR admins')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleToggleStatus = async (adminId, currentStatus) => {
    try {
      ...
      if (currentStatus) {
        await adminService.deactivateHRAdmin(adminId)
      } else {
        await adminService.activateHRAdmin(adminId)
      }
      await fetchHRAdmins()
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      setActionLoading(true)
      await adminService.deleteHRAdmin(selectedAdmin.user_id)
  ...
  const filteredAdmins = hrAdmins.filter(admin => 
    ...
    return (
      <div className="font-display min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-teal-50 flex items-center justify-center">
        <div className="flex flex-col items-center space-y-6">
          ...
          <p className="text-slate-600 font-medium">Loading HR Admins...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="font-display min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-teal-50/20">
        ...
        <div className="max-w-[1400px] mx-auto mt-4 rounded-2xl px-6 py-3 flex items-center justify-between bg-white/80 backdrop-blur-2xl border border-white/40 shadow-xl shadow-slate-200/50">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-slate-800 text-lg font-bold tracking-tight">HR Admins</h2>
          ...
          <div className="flex items-center gap-3">
            <Link
              ...
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-teal-500 to-teal-600 text-white rounded-xl text-sm font-bold hover:from-teal-600 hover:to-teal-700 transition-all shadow-lg shadow-teal-500/25"
            >
              <UserPlus className="h-4 w-4" />
              <span className="hidden sm:inline">Create HR Admin</span>
            </Link>
          </div>
        </div>
      </header>
      ...
      <main className="pt-24 pb-8 px-4 lg:px-8">
        <div className="max-w-[1400px] mx-auto">
          {/* Page Title */}
          <div className="mb-8">
            <h1 className="text-3xl font-black text-slate-800 mb-2">HR Administrators</h1>
            <p className="text-slate-500">Manage and monitor HR administrator accounts</p>
          </div>
              ...
              <div className="flex-1">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3 rounded-xl border-2 border-slate-100 bg-slate-50 focus:border-teal-500 focus:bg-white transition-all text-sm font-medium text-slate-700 placeholder:text-slate-400"
                  />
                </div>
              </div>
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/40 shadow-xl shadow-slate-200/50 overflow-hidden">
            {filteredAdmins.length > 0 ? (
                    ...
                    {filteredAdmins.map((admin) => (
                      <tr key={admin.user_id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center shadow-md">
              ...
              <div className="px-6 py-20 text-center">
                ...
                <h3 className="text-xl font-bold text-slate-800 mb-2">No HR admins found</h3>
                <p className="text-slate-500 mb-6 max-w-sm mx-auto">
                  {searchTerm ? 'No admins match your search criteria.' : 'Start by creating your first HR administrator account.'}
                </p>
                <Link
                  to="/admin/hr-admins/create"
                  className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-teal-500 to-teal-600 text-white rounded-xl font-bold hover:from-teal-600 hover:to-teal-700 transition-all shadow-lg shadow-teal-500/25"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 border border-slate-100">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className="text-xl font-bold text-slate-800">Delete HR Admin</h3>
