...
class LoanModel {
  ...
  static async checkEligibility(userId) {
    ...
    const user = await query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM loan_applications WHERE user_id = ? AND status IN ('PENDING', 'UNDER_REVIEW', 'APPROVED')) as pending_applications
      FROM users u
      LEFT JOIN employee_profiles ep ON u.id = ep.user_id
      WHERE u.id = ?
    ...
    if (user && user.length > 0) {
      ...
      console.log('Email verified:', user[0].email_verified);
      console.log('Active loans:', user[0].active_loans);
      console.log('Pending applications:', user[0].pending_applications);
    }
    
    if (!user || user.length === 0) {
      console.log('❌ User not found or missing employee profile');
    ...
    if (!userData.employment_status) {
      console.log('❌ Employee profile required');
    ...
    if (userData.employment_status !== 'ACTIVE') {
      console.log('❌ Employment status not active:', userData.employment_status);
    ...
    if (userData.active_loans > 0) {
      console.log('❌ Maximum active loans reached:', userData.active_loans);
      return { eligible: false, reason: 'You already have an active loan. Please complete it before applying for a new one.' };
    }
    
    if (userData.pending_applications > 0) {
      console.log('❌ Has active/pending applications:', userData.pending_applications);
      return { eligible: false, reason: 'You already have an active or pending loan application. Please complete or wait for it to be processed before applying again.' };
    }
    ...
    console.log('Financials query result:', financials);
    console.log('Salary:', financials?.[0]?.salary);
    console.log('Savings balance:', financials?.[0]?.savings_balance);
    ...
    const result = {
      ...
      user_data: userData,
      financials: {
        salary: parseFloat(financials?.[0]?.salary || 0),
        savings_balance: parseFloat(financials?.[0]?.savings_balance || 0),
        max_loan_by_savings: parseFloat(financials?.[0]?.savings_balance || 0) * 2,
        max_monthly_repayment: parseFloat(financials?.[0]?.salary || 0) * 0.33
      }
    };
  ...
  static async checkGuarantorCapacity(guarantorEmployeeId, requestedAmount, applicantId) {
    console.log('🔍 Checking guarantor capacity for:', { guarantorEmployeeId, requestedAmount, applicantId });
    ...
    console.log('Guarantor query result:', results);

    const guarantor = results[0];

    if (!guarantor) {
      console.log('❌ Guarantor not found or inactive');
      return { eligible: false, reason: 'Guarantor employee not found or inactive' };
    }

    console.log('Guarantor found:', { user_id: guarantor.user_id, name: `${guarantor.first_name} ${guarantor.last_name}`, salary: guarantor.salary, active_guarantees: guarantor.active_guarantees });

    if (guarantor.user_id === applicantId) {
      console.log('❌ Self-guarantor attempt');
      return { eligible: false, reason: 'You cannot be your own guarantor' };
    }

    if (guarantor.active_guarantees >= 3) {
      console.log('❌ Too many active guarantees:', guarantor.active_guarantees);
      return { eligible: false, reason: 'Guarantor has reached maximum active guarantees (3)' };
    }


    const maxGuaranteeCapacity = guarantor.salary * 5;
    console.log('Guarantor capacity check:', { requestedAmount, maxGuaranteeCapacity, salary: guarantor.salary });

    if (requestedAmount > maxGuaranteeCapacity) {
       console.log('❌ Requested amount exceeds guarantor capacity');
       return { eligible: false, reason: `Requested amount exceeds guarantor's capacity based on salary` };
    }

    console.log('✅ Guarantor eligible');
    return {
      eligible: true,
      guarantor_data: {
        id: guarantor.user_id,
