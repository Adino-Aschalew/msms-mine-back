...
class PdfUtils {
  ...
  static async generateIndividualPayslip(payslipData, outputPath = null) {
    const template = {
      title: 'Monthly Payslip',
      description: 'Employee Monthly Earnings and Deductions Statement',
      header: {
        title: 'PAYSLIP',
      ...
      content: [
        ...
        },
        {
          type: 'table',
          headers: [
            { text: 'Description', key: 'description', width: 250 },
            { text: 'Amount (ETB)', key: 'amount', width: 150, align: 'right' }
          ],
          rows: payslipData.items,
          style: {
            headerStyle: { font: 'Helvetica-Bold' },
            borderWidth: 1
          }
    ...
    if (outputPath) {
      return this.generateReport(payslipData, template, outputPath);
    }
    return this.generatePdfBuffer(payslipData, template);
  }

  static async generateLoanAgreement(agreementData, outputPath = null) {
    const template = {
      title: 'Loan Agreement',
      description: 'Official Loan Agreement and Terms',
      header: {
        title: 'LOAN AGREEMENT',
        subtitle: 'Agreement No: {{agreementNumber}}',
        date: '{{date}}'
      },
      content: [
        {
          type: 'text',
          text: 'This loan agreement is made between {{companyName}} (Lender) and {{borrowerName}} (Borrower), Employee ID: {{employeeId}}.',
          spacing: 2
        },
        {
          type: 'text',
        ...
        },
        {
          type: 'list',
          items: [
            'Principal Amount: {{loanAmount}} ETB',
            'Interest Rate: {{interestRate}}% per annum',
            'Loan Duration: {{duration}} months',
            'Monthly Repayment: {{monthlyPayment}} ETB',
        ...
        {
          type: 'text',
          text: 'The borrower agrees to have the monthly repayment amount deducted from their monthly salary until the loan is fully repaid. Any default in payment will be handled according to the institution\'s credit policy.',
          spacing: 2
        },
        {
          type: 'text',
          text: '3. Repayment Schedule Summary:',
          style: 'subheading'
        },
        {
          type: 'table',
          headers: [
            { text: 'Month', key: 'month', width: 50 },
            { text: 'Repayment', key: 'amount', width: 100 },
            { text: 'Principal', key: 'principal', width: 100 },
            { text: 'Interest', key: 'interest', width: 100 },
            { text: 'Balance', key: 'balance', width: 100 }
          ],
          rows: agreementData.schedule,
          style: {
            headerStyle: { font: 'Helvetica-Bold' },
            borderWidth: 1
          }
      ...
      footer: {
        text: 'Generated by MSMS Microfinance System',
        pageNumber: true,
        company: '{{companyName}}'
      }
    };
    ...
    if (outputPath) {
      return this.generateReport(agreementData, template, outputPath);
    }
    return this.generatePdfBuffer(agreementData, template);
  }
